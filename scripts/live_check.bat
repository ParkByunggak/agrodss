...
REM it in Notepad. Values - PNU, soil, prescription - stay in data\soil\ which is git-ignored. Paste the Notepad text into the session.
REM
...
REM cultivation-unit id via --subject, never written here - the address is PII. Delayed expansion so that the exit code
REM inside the block is the real one - plain %errorlevel% is expanded when the block is parsed.
REM [publisher live 2026-09-19 21:30] NO PARENTHESES inside the redirected block: a ")" in an echo line closes the block
REM early, cmd reports a syntax error, nothing runs and Notepad opens an empty log. That is exactly what happened.
...
echo === agrodss live check %date% %time% === > "%LOG%"
echo python = %PY% >> "%LOG%"
