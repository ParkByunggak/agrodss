...
#
# 인증 갈래(H): cert=유기 → 공시 유기농업자재. cert=관행 → PSIS 등록약제(아직 인용 전 → 해당 없음 + 사유).
#   범주명("BT제")으로 메우지 않는다 — 공시 목록에 없으면 no_data 로 정직하게 비운다.
...
registry.register(registry.Decision(
    ...
    rule=("cert=유기이면 오늘이 속한 격자 칸의 작업 자재(유기 갈래) 계열마다 농관원 공시 목록에서 유효 제품을 인용한다. "
          ...
          "cert=관행이면 해당 없음(PSIS 인용 전). 인용은 판단이 아니다 — 효능 보증 아님을 함께 싣는다."),
    revisit_days=7,
    params={
        "per_alias_limit": 3,
...
def judge(subject: dict[str, Any], today: date | None = None) -> Envelope:
    ...
    if cert != "유기":
        return Envelope("해당 없음", DECISION_ID, sid, as_of,
                        result={"why": f"인증 유형 {cert!r} — 공시 유기농업자재 대상이 아니다. 관행은 PSIS 등록약제(M-15 ⑤, 인용 전)"})
